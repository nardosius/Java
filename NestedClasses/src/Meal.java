...
private double price;
    ...
    public Meal() {//conversion rate initialized to 1
        this(1);
    }

    public Meal(double conversionRate) {//constructor taking conversion rate as argument
...
private List<Item> toppings = new ArrayList<>();
        ...
        private void addToppings(String... selectedToppings) {//this allows selectedToppings to be treated like array
            for (String s : selectedToppings) {
                try {
                    Extra topping = Extra.valueOf(s.toUpperCase());
                    toppings.add(new Item(topping.name(), "TOPPING", topping.getPrice()));
                } catch (IllegalArgumentException ie) {
                    System.out.println("No topping found for " + s);
                }
            }
        }
